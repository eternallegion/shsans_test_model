...
def run(rank, n_gpus, hps):
  ...
  #   #   # 
  #   #   #   # Fine-tuning: Freeze encoder, flow, duration predictor
  #   #   #   #for name, param in net_g.named_parameters():
  #   #   #   #    if "enc_p" in name or "flow" in name or "dp" in name:
  #   #   #   #        param.requires_grad = False
  #   #   # 
  #   #   #   #for name, p in net_g.named_parameters():
  print("[INFO] ===== ALL LAYERS TRAINABLE (NO FREEZE) =====")
  #    if not p.requires_grad:
  #        print("[FREEZE]", name)
  print("[INFO] All layers are trainable (UNFROZEN)")
  ...
  """for name, param in net_g.named_parameters():
    if (
        "enc_p" in name or    # Text Encoder
        "flow" in name or     # Flow
        "dp" in name          # Duration Predictor
    ):
        param.requires_grad = False
  ...
  """try:
    ...
    _, _, _, epoch_g = utils.load_checkpoint(g_path, net_g, optim_g)
    _, _, _, epoch_d = utils.load_checkpoint(d_path, net_d, optim_d)

    assert epoch_g == epoch_d

    epoch_str = epoch_g
    global_step = (epoch_str - 1) * len(train_loader)

    logger.info(f"Resumed from epoch {epoch_str}, step {global_step}")
